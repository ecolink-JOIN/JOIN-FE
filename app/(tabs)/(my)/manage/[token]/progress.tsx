...
import { FlatList, View, Alert, ActivityIndicator, TouchableOpacity, Modal, Text } from 'react-native';
...
const Progress = ({ bottomSheetModalRef }: { bottomSheetModalRef: React.RefObject<BottomSheetModalMethods> }) => {
  ...
  const toggleModal = () => {
    console.log('🟡 스터디 종료 BottomSheet 열기 - toggleModal 호출됨');
    Alert.alert('테스트', 'toggleModal이 호출되었습니다!');
    closeBottomSheetRef.current?.present();
    console.log('🟡 present() 호출 완료');
  };

  const handleCloseStudy = () => {
    console.log('🔴 스터디 종료 버튼 클릭됨 - 함수 진입');
    Alert.alert('스터디 종료', '정말 스터디를 종료하시겠습니까?\n종료된 스터디는 되돌릴 수 없습니다.', [
      ...
      },
      {
        ...
        style: 'destructive',
        onPress: async () => {
          console.log('🔴 실제 종료 시작');
          try {
            setIsClosing(true);
            const today = new Date().toISOString().split('T')[0];

            console.log('🔴 API 호출 시작:', token, today);
  ...
  return (
    <>
      <ManageView>
        ...
        />
        <BottomSheetComp
          bottomSheetModalRef={closeBottomSheetRef}
          component={
            <View style={{ padding: 20, gap: 12 }}>
              ...
              <Typography variant="body4" style={{ textAlign: 'center' }}>
                {studyDetail && new Date(studyDetail.endDate) > new Date()
                  ? '설정된 스터디 기간이 남아있습니다.\n'
                  : ''}
                정말 종료하시겠습니까?
              </Typography>
